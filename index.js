...
/*  (oto test yok) sayilar dizisi içinde kaç adet sayı olduğunu konsola yazdırın */
	...
	let yeniSira = sayilar.sort((a, b) => a - b);

	let newObj = {};

	for (let i = 0; i < yeniSira.length; i++) {
		
		if (yeniSira[i] in newObj) {
	...
	for (let key in newObj) {
		...
		if (newObj[key] > 1) {
			...
			tekraredensayilar.push(key + " sayısı " + newObj[key] + " kere tekrar edilmiştir")
		}
	}
